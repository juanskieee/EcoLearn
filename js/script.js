...
function showTimeUpModal() {
    ...
    const titleEl = document.getElementById('timeup-title');
    const mainEl = document.getElementById('timeup-main-text');
    const subEl = document.getElementById('timeup-sub-text');

    if (sessionMode === 'instructional') {
        if (titleEl) titleEl.textContent = '🌿 Learning session complete!';
        if (mainEl) mainEl.textContent = 'Nice work exploring the eco-cards.';
        if (subEl) subEl.textContent = 'We will now show your learning summary.';
    } else {
        if (titleEl) titleEl.textContent = '⏰ Time is up!';
        if (mainEl) mainEl.textContent = 'Your test session has ended.';
        if (subEl) subEl.textContent = 'We will now show your results and leaderboard rank.';
    }
